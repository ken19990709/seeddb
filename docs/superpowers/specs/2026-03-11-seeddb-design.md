...
| P0-5 错误处理框架 (error.h) | 0.5 天 | 错误码定义 + try-catch 测试通过 |
...
1. **立即开始**：创建项目目录结构和 CMake 配置
2. **第一个里程碑**：Phase 1 完成，能在内存中执行基础 SQL
3. **第二个里程碑**：Phase 2 完成，能用 psql 连接
4. **第三个里程碑**：Phase 3 完成，数据持久化到磁盘
5. **第四个里程碑**：Phase 4 完成，支持完整事务
